...
## Redis
Redis is a cache of PostgreSQL truth. At API startup, and via `POST /api/admin/rebuild-availability`, every future night that has active bookings is recomputed from the `bookings` table and written back, so a restarted or flushed Redis cannot cause overselling.

`availability:<roomId>:<YYYY-MM-DD>` stores the remaining inventory of a room for one night. Keys are created lazily from `rooms.total_rooms` the first time a night is read or booked.

Booking a date range runs one Lua script over every night of the stay (check-in inclusive, check-out exclusive): if any night is sold out it returns -1 and changes nothing, otherwise it decrements all nights atomically. Cancelling runs the inverse script, incrementing each night but never above `total_rooms`. This is what prevents overselling under concurrent requests without a database lock.
